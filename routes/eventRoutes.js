...
router.get('/:name/next', getSpotifyClient, async (req, res) => {
  ...
  try {
    ...
    console.log('next item id', nextSongInQueue);
...
router.post('/:name/playlist/reorder', async (req, res) => {
  ...
  // Get the count of historical tracks
  const historicalCount = await prisma.queue.count({
    where: {
    ...
    tracks
      .map((track, index) =>
        track.position > 0
          ? prisma.queue.update({
              where: { id: track.id },
              data: { position: index + 1 }, // Add the count of historical tracks
            })
          : null,
      )
      .filter(Boolean),
